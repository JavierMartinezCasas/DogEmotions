...
sdir = r'../input/dog-emotions-prediction/images'
...
def make_model(img_size, lr, mod_num=3):  # defaults to EfficientNet B3 model
    ...
    if mod_num == 0:
        ...
        base_model = tf.keras.applications.MobileNetV3Large(input_shape=img_shape, include_top=False, classes=None,
                                                            pooling='max', dropout_rate=0.2)
        msg = 'Created MobileNet model'
    elif mod_num == 2:
        base_model = tf.keras.applications.InceptionResNetV2(input_shape=img_shape, include_top=False, classes=None,
                                                             pooling='max')
        msg = 'Created Inception-Resnet Model'
    elif mod_num == 3:
...
lr = .001
emodel = make_model(img_size, lr)  # using B3 model by default
            ...
            if epoch + 1 == self.ask_epoch:  # is this epoch the one for quering the user?
                ...
                if ans == 'H' or ans == 'h' or ans == '0':  # quit training for these conditions
                    ...
                    if self.ask_epoch > self.epochs:
                        ...
                        if self.dwell == False:
                            lr = float(
                                tf.keras.backend.get_value(self.model.optimizer.lr))  # get the current learning rate
...
epochs = 40
ask_epoch = 5
ask = LR_ASK(emodel, epochs, ask_epoch)
callbacks = [ask]

history = emodel.fit(x=train_gen, epochs=epochs, verbose=1, callbacks=callbacks, validation_data=valid_gen,
...
def predictor(test_gen, model):
    ...
    return errors, tests, error_list, f1score, preds


errors, tests, error_list, ef1score, epreds = predictor(test_gen, emodel)
...
# Save the EfficentNet model
subject = 'EfficientNet'
emodel_loc = save_model(emodel, subject, classes, img_size, ef1score, working_dir)

mmodel = make_model(img_size, lr, 1)
ask = LR_ASK(mmodel, epochs, ask_epoch)
callbacks = [ask]
train_gen, test_gen, valid_gen, test_steps = make_gens(batch_size, train_df, test_df, valid_df, img_size)
history = mmodel.fit(x=train_gen, epochs=epochs, verbose=1, callbacks=callbacks, validation_data=valid_gen,
                     validation_steps=None, shuffle=False, initial_epoch=0)
loss_index = tr_plot(history, 0)
errors, tests, error_list, f1score, mpreds = predictor(test_gen, mmodel)
subject = 'MobileNet'
mmodel_loc = save_model(mmodel, subject, classes, img_size, f1score, working_dir)


def ave_predictor(m1preds, m2preds, gen):
    y_pred = []
    error_list = []
    error_pred_list = []
    y_true = gen.labels
    ...
    for i in range(class_count):
        empty.append(0)
    for i in range(len(m1preds)):
        psum = empty
        for j in range(class_count):
            psum[j] = (m1preds[i][j] + m2preds[i][j]) / 2
        pred_index = np.argmax(psum)
    ...
    clr = classification_report(y_true, y_pred, target_names=classes, digits=4)  # create classification report
    print("Classification Report:\n----------------------\n", clr)
    return ytrue, ypred, f1score


aytrue, aypred, afiscore = ave_predictor(epreds, mpreds, test_gen)
